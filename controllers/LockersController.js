...
const {
  runValidation, required, mustBeString, mustBeOneOf, mustBePositiveInt,
...
class LockersController {
  ...
  // GET /api/lockers  –  ?search=  ?sort=  ?status=
  async getAll(req, res) {
    try {
      ...
      if (search) { conditions.push('locker_code LIKE ?'); params.push(`%${search}%`); }
  ...
  async getOne(req, res) {
    try {
      ...
      if (rows.length === 0)
        return res.status(404).json({ status: 'error', message: 'Locker not found' });
  ...
  async create(req, res) {
    try {
      ...
      const errors = runValidation([
        { field: 'locker_code',  value: locker_code,  checks: [required, mustBeString] },
        { field: 'size',         value: size,         checks: [required, mustBeOneOf(SIZES)] },
        { field: 'status',       value: status,       checks: [mustBeOneOf(STATUSES)] },
        { field: 'location_id',  value: location_id,  checks: [mustBePositiveInt] },
      ]);
      if (errors.length)
      ...
      const [result] = await db.query(
        ...
        [locker_code.trim(), size, status || 'available', location_id || null]
      );
      const [rows] = await db.query('SELECT * FROM lockers WHERE locker_id=?', [result.insertId]);
      res.status(201).json({ status: 'success', data: rows[0] });
    } catch (err) {
      if (err.code === 'ER_DUP_ENTRY')
        return res.status(409).json({ status: 'error', message: 'Locker code already exists' });
  ...
  async update(req, res) {
    try {
      ...
      const [result] = await db.query(
        ...
        [locker_code.trim(), size, status, location_id || null, req.params.id]
      );
